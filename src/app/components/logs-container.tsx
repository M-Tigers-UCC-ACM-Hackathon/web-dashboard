...
const LogsContainer: React.FC<LogsContainerProps> = ({
    ...
    const fetchTotalCounts = async () => {
        try {
            ...
            if (data.nginxLogsCount !== undefined) {
                setTotalLogs(data.nginxLogsCount);
            }
        } catch (error) {
    ...
    return (
        <Card className="border border-gray-300">
            ...
            <CardContent>
                {(!isLoadingInitial && logs.length === 0 && !error) && <p>No logs to display.</p>}
                {(isLoadingInitial || logs.length > 0) && <LogsTable data={tableData} />}
            </CardContent>
        </Card>
    );
};
